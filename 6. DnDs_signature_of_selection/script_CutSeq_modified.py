...
for line in GFFfile:
	...
	if Gene != geneName:
		if Gene != "":
			...
			if Region == "CDS":
				for name, seq in read_fasta(fp):
                    			if len(goterms)==0:
                        			print(name+" | "+Gene+ " | "+description, file=fasta)
                    			else:
                        			print(name+" | "+Gene+ " | "+description+" | "+goterms[0], file=fasta)
					if revc == "+":
	...
	revc = arrline[6]
	beg.append(int(arrline[3])-1)
	end.append(int(arrline[4]))
	phase.append(int(arrline[7]))

    	attributes_split = attributes.split(';')
    	description = attributes_split[2]
    	goterms = [i for i in attributes_split if i.startswith('Ontology_id')]
