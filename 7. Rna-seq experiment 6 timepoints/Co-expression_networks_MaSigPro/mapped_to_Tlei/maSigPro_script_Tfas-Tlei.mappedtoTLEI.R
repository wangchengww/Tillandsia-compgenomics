...
write.table(sigs$summary$TfasvsTlei, file = "Genes_Significant_Tfas-vs-Tlei_0.7-trimmed.exonic.mapped-to-Tlei.txt", quote = F, sep = "\t", row.names = F)
...
write.table(genes_underexpressedTlei, "Genelist_UnderExpressed_Tlei.EXONIC.txt",
            quote = F, row.names = F)
save(sigs, d, normd, NBp, NBt, file = "maSigPro_data_run_Tfas-vs-Tlei_0.7-trimmed.exonic.mapped-to-Tlei.RData")
