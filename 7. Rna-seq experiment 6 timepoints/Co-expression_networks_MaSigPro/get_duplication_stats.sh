echo "Module total one_one larger_Tfas larger_Tlei equal uniq_Tfas uniq_Tlei" > duplication_stats.per-orthogroup.txt
for i in Genes_Significant_Tfas-vs-Tlei_0.7-trimmed_TLEI-REF-cluster*; do
  grep -f $i orthogroup-info_Significant_Genes-Tfas-vs-Tlei_0.7-trimmed_TLEI-REF.size-corrected.noplastid-mito.txt > list.txt
  cut -f 7 list.txt > list.ID.txt
  grep -f list.ID.txt orthogroup-counts_Significant_Genes-Tfas-vs-Tlei_0.7-trimmed_TLEI-REF.size-corrected.noplastid-mito.txt > og_list.txt
  ...
  n=${n##*-}
  echo $n
  echo $n $total $oneOne_p $larger_Tfas_p $larger_Tlei_p $equal_p $uniq_Tfas_p $uniq_Tlei_p >> duplication_stats.per-orthogroup.txt
